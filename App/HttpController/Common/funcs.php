 ...
 * 通过聊天组标识去换取id
 */
function common_trans_chat_group_no_to_chat_group_id($chat_group_no,$chat_group_type='single_chat'){
    return \EasySwoole\RedisPool\Redis::invoke('redis', function (\EasySwoole\Redis\Redis $redis) use ($chat_group_no,$chat_group_type) {
        // $chat_group_id = $redis->get('trans_chat_group_no_'.$chat_group_no);
        // if($chat_group_id > 0){
        //     return $chat_group_id;
        // }else{
            // 去数据库里查找
            $chat_group_id = \App\HttpController\Chat\Model\ChatGroupModel::create()->where('chat_group_no',$chat_group_no)->val('id');
            if($chat_group_id > 0){
                $redis->set('trans_chat_group_no_'.$chat_group_no,$chat_group_id,3600);
                return $chat_group_id;
            }else{
                return 0;
            }
