...
class WebSocketParser implements ParserInterface
{
    ...
    public function decode($raw, $client) : ? Caller
    {
        ...
        // if (strtolower($raw) !== 'ping') {
        //     $payload = json_decode($raw, true);
        //     $class   = isset($payload['class']) ? $payload['class'] : 'index';
        //     $action  = isset($payload['type']) ? $payload['type'] : 'actionNotFound';
        //     $params  = isset($payload['data']) ? (array)$payload['data'] : [];
        //     $controllerClass = "\\App\\WebSocket\\" . ucfirst($class);
        //     if (!class_exists($controllerClass)) $controllerClass = "\\App\\WebSocket\\Index";
        //     $caller->setClient($caller);
        //     $caller->setControllerClass($controllerClass);
        //     $caller->setAction($action);
        //     $caller->setArgs($params);
        // } else {
        //     $caller->setControllerClass("\\App\\WebSocket\\Index");
        //     $caller->setAction('heartbeat');
        // }
        // return $caller;

        if (strtolower($raw) === 'ping' || strtolower($raw) === 'heart') {
            $caller->setControllerClass("\\App\\WebSocket\\Index");
            $caller->setAction('heartbeat');
            return $caller;
        }
